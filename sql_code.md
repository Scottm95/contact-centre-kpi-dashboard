...
CREATE TABLE fact_calls AS SELECT
...
-- Find total calls, calls with/without handle time
SELECT 
    COUNT(*) AS total_calls,
...
-- Create buckets for calls depending on handle time  
WITH bucketed AS (
    SELECT 
        CASE 
...
-- Average Queue Time: Abandoned vs Answered
SELECT
    CASE 
